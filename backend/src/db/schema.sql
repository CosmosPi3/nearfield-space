...
-- exploration, their role, and discovery provenance). No FK enforcement —
-- this DB runs with foreign_keys OFF, and a node must be persistable the
-- instant it's added client-side, before any matching `tracks` row
-- necessarily exists yet.
CREATE TABLE IF NOT EXISTS workspace_nodes (
  id            TEXT PRIMARY KEY REFERENCES tracks(id),   -- not enforced; documentation only
...
CREATE INDEX IF NOT EXISTS idx_workspace_nodes_via_id ON workspace_nodes(via_id);
...
CREATE INDEX IF NOT EXISTS idx_workspace_discoveries_parent_id ON workspace_discoveries(parent_id);
...
INSERT INTO workspace_discoveries (child_id, parent_id, similarity, cosine_score, discovered_at)
SELECT id, via_id, similarity, cosine_score, added_at
